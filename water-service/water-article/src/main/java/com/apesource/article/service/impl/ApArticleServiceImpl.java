...
import com.ape.model.article.pojos.ApArticleContent;
...
import com.apesource.article.mapper.ApArticleContentMapper;
import com.apesource.article.mapper.ApArticleMapper;
import com.apesource.article.service.ApArticleService;
import com.apesource.article.service.ArticleFreemarkerService;
...
public class ApArticleServiceImpl extends ServiceImpl<ApArticleMapper, ApArticle> implements ApArticleService {
    ...
    public ResponseResult delArticle(Long id){
        if (id!=0){
            int a = apArticleMapper.deleteById(id);
            int b = apArticleConfigMapper.delete(Wrappers.<ApArticleConfig>lambdaQuery().in(ApArticleConfig::getArticleId, id));
            int c = apArticleContentMapper.deleteById(id);
            if (a>0 && b>0 && c>0){
                return ResponseResult.okResult("删除成功！");
            }
        }
        return ResponseResult.errorResult(1002,"操作失败！");
    }
}
